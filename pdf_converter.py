...
# Force ReportLab to be available since we've installed it
...
def generate_expense_document(expense_data):
    ...
    try:
        ...
        try:
            ...
            epv_id = data.get('epv_id', 'EPV-' + datetime.now().strftime('%Y%m%d') + '-' + uuid.uuid4().hex[:10].upper())
            ...
            header_data = [
                [Paragraph("Expense Report", title_style), Paragraph(f"Date: {today_date}", normal_style)],
            ...
            info_data = [
                ...
                [Paragraph("<b>From Date:</b>", normal_style), data.get('from_date', ''),
                 Paragraph("<b>To Date:</b>", normal_style), data.get('to_date', '')],
            ]
            ...
            expense_table = Table(expense_data, colWidths=[0.5*inch, 1*inch, 1.5*inch, 3*inch, 1*inch])
            ...
            # Add a large spacer to push amount in words and signatures to bottom
            elements.append(Spacer(1, 3*inch))

            # Add amount in words at the bottom of the page
            elements.append(Paragraph(f"<b>Amount in words:</b> {data.get('amount_in_words', '')}", normal_style))
            elements.append(Spacer(1, 0.5*inch))
